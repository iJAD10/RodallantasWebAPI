...
namespace API_Roda_Llantas.Models
{
	public class ReservacionesModel : IReservacionesModel
	{
		...
		public int RegistrarVehiculoYReservacion(ReservacionesEntities entidad)
		{
			using (var conexion = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
			{
				int reservacion = conexion.Query<int>("RegistrarVehiculoYReservacion",
			new
			{
				Res_Usuario_Id = entidad.Res_Usuario_Id,
				Veh_Fecha_Lanzamiento = entidad.Veh_Fecha_Lanzamiento,
				Veh_Marca = entidad.Veh_Marca,
				Veh_Modelo = entidad.Veh_Modelo,
				Veh_Placa = entidad.Veh_Id
			},
